...
const InsuranceFinancialStatusFormScreen = () => {
    const localSearchParams = useLocalSearchParams()
    return <InsuranceFinancialStatusFormScreenComponent localSearchParams={localSearchParams} />
}

export default InsuranceFinancialStatusFormScreen

class InsuranceFinancialStatusFormScreenComponent extends React.Component<any, any> {
  ...
  getInsuranceDetails = async (ins_id) => {
    try {
      axios.get(`${Config.api}/insurance/getInsurance/${ins_id}`)
        .then(res => {
          console.log('insurance details: ', res.data.result.fsFields);
          this.setState({
  ...
  getLoanSubmitStatus = (currentOngoingLoans, retailer_id) => {
            ...
            if (data.soc == 'Employed') {
              ...
              fsFields.map(f => {
                if (this.state.userData.dynamicFinancialStatus && this.state.userData.dynamicFinancialStatus.data) {
                  this.state.userData.dynamicFinancialStatus.data.map(userSavedPI => {
                    console.log('--------------------------------');
  ...
  submitForm = async () => {
    ...
    let data = {
      ...
      insurance: this.state.insurance._id,
      soc: state.soc,
  ...
  checker = () => {
    try {
      ...
      if (this.state.soc == 'Employed') {
        this.state.fsFields.map(field => {
          if ((field.isShow && field.isRequired) && (field.value == null || field.value == "")) {
            returnValue = true
          }
        })
      } else {
        this.state.fsSelfEmployedFields.map(field => {
          if ((field.isShow && field.isRequired) && (field.value == null || field.value == "")) {
            returnValue = true
          }
        })
          ...
          <ScrollView>
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
              ...
              <View style={globalStyle.wrapper}>
                ...
                <Text style={globalStyle.title}>Insurance Financial Status</Text>
                {
